...
class DDPMSampler:
    ...
    def add_noise(
        ...
        edit_latents_at_t = dict()
        if edit_timesteps != None:
            for timestep in edit_timesteps:
                sqrt_alpha_prod = alphas_cumprod[timestep] ** 0.5
                sqrt_alpha_prod = sqrt_alpha_prod.flatten()
                while len(sqrt_alpha_prod.shape) < len(noisy_samples.shape):
                    sqrt_alpha_prod = sqrt_alpha_prod.unsqueeze(-1)
                ...
                while len(sqrt_one_minus_alpha_prod.shape) < len(noisy_samples.shape):
                    sqrt_one_minus_alpha_prod = sqrt_one_minus_alpha_prod.unsqueeze(-1)
                edit_latents_at_t[int(timestep.item())]=sqrt_alpha_prod * original_samples + sqrt_one_minus_alpha_prod * noise

        return noisy_samples, noised_latents_at_t, edit_latents_at_t
