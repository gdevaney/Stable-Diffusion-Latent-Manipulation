...
def generate(
    ...
    with torch.no_grad():
        ...
        if input_image:
            ...
            edit_timesteps, other_timesteps = timestamps_to_manipulate(sampler)
            print(f"Timesteps for manipulating latent space: {other_timesteps}")
            latents, noised_latents, edit_latents = sampler.add_noise(latents, sampler.timesteps[0], other_timesteps, edit_timesteps)
            noised_latents[999] = torch.randn(latents_shape, generator=generator, device=device)
            to_idle(encoder)
        else:
            # (Batch_Size, 4, Latents_Height, Latents_Width)
        ...
        for i, timestep in enumerate(timesteps):
            ...
            print(noised_latents.keys())
            if i < len(sampler.timesteps) - 1 and int(sampler.timesteps[i+1]) in edit_latents.keys():            
                ...
                # cumulative_attention_maps2 = cumulative_subject_attention_map(cumulative_attention_maps)
                visualize_cumulative_map(cumulative_attention_maps, int(sampler.timesteps[i+1]))
                
                diffusion.set_attention_map({})
                # print(noised_latents[int(sampler.timesteps[i+1])])

                latents = latent_space_manipulation(latents, noised_latents, cumulative_attention_maps, zz, focus)
                zz+=1
